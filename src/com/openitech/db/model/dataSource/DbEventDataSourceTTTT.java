 ...
 * $Revision: 1.8 $
 */
package com.openitech.db.model.dataSource;

import com.openitech.db.model.DbDataSource.SqlParameter;
import com.openitech.db.model.sql.*;
 ...
 * @author uros
 */
public class DbEventDataSourceTTTT implements DbDataSourceImpl {
  ...
  public DbEventDataSourceTTTT(DbDataSource owner) {
    this.owner = owner;
  }
...
//      return openSelectResultSet().findColumn(columnName);
  ...
  protected void createCurrentResultSet() {
    ...
    try {
      ...
      try {
        ...
        currentResultSet = new CurrentResultSet(owner.getParameters());
  ...
  public int getColumnIndex(String columnName) throws SQLException {
    ...
    if (columnMapping.containsKey(columnName)) {
      ...
      try {
        ...
        if (ci == -1) {
          Logger.getLogger(DbEventDataSourceTTTT.class.getName()).log(Level.WARNING, "Invalid column name [" + columnName + "]");
        }
      }
  ...
  private class CurrentResultSet {
    ...
    public CurrentResultSet(List<Object> parameters) throws SQLException {
      this.currentResultSet = new DbWebRowSetImpl();
      this.currentResultSet.setFetchSize(getFetchSize());
      setParameters(parameters);
      ((WebRowSet) this.currentResultSet).execute();

    }

    private void close() throws SQLException {
      ((WebRowSet) currentResultSet).release();

    }

    private void setParameters(List<Object> parameters) throws SQLException {
      int param = 1;
//      ((WebRowSet) this.currentResultSet).setInt(param++, -1);//1154275);
//      ((WebRowSet) this.currentResultSet).setInt(param++, 70);
//      ((WebRowSet) this.currentResultSet).setString(param++, "PIK01");
      DbDataSource.SqlParameter eventID = (SqlParameter) parameters.get(0);
      if (eventID != null && eventID.getValue() != null) {

        ((WebRowSet) this.currentResultSet).setLong(param++, (Long) eventID.getValue());
      } else {
        ((WebRowSet) this.currentResultSet).setNull(param++, java.sql.Types.INTEGER);
      }
      Integer idSifranta = (Integer) parameters.get(1);
      if (idSifranta != null) {

        ((WebRowSet) this.currentResultSet).setInt(param++, idSifranta.intValue());
      } else {
        ((WebRowSet) this.currentResultSet).setNull(param++, java.sql.Types.INTEGER);
      }
      ((WebRowSet) this.currentResultSet).setString(param++, (String) parameters.get(2));
  ...
  public boolean loadData(boolean reload, int oldRow) {
    ...
    if (reload) {
      ...
      try {
        ...
        Logger.getLogger(Settings.LOGGER).log(Level.WARNING, "Can't properly close the for ", ex);
      } finally {
        currentResultSet = null;
        owner.unlock();
      }
    ...
    if ((currentResultSet == null)) {
      ...
      try {
        ...
        reloaded = true;
        owner.unlock();
        Logger.getLogger(Settings.LOGGER).finer("Permit unlockd ''");
      }
    ...
    if (reloaded && currentResultSet != null) {
      if (EventQueue.isDispatchThread() || !owner.isSafeMode()) {
        ...
        try {
          ...
          Logger.getLogger(Settings.LOGGER).log(Level.SEVERE, "Can't notify loaddata results from ''", ex);
        }
      }
    }
    return currentResultSet != null;
      ...
      if (wasUpdated(rowIndex, columnName)) {
        ...
        if (rowIndex > getRowCount()) {
          ...
          try {
            ...
            openSelectResultSet.absolute(rowIndex);
            ;
  ...
  public String getDataSourceName() {
    if (loadData()) {
      try {
        ...
        Logger.getLogger(DbEventDataSourceTTTT.class.getName()).log(Level.SEVERE, null, ex);
      }
    }
  ...
  private class DbDataSourceHashMap<K, V> extends HashMap<K, V> {
    ...
    public V checkedGet(Object key) throws SQLException {
      ...
      if (containsKey(key)) {
        ...
        throw new SQLException("DbDataSource for '' does not contain '" + key.toString() + "'.");
      }
    }
  ...
  private static class CacheEntry<K, V> extends SoftReference<DbEventDataSourceTTTT> {

    K key;
    V value;

    private CacheEntry(DbEventDataSourceTTTT referent, K key, V value) {
      super(referent);
      this.key = key;
      this.value = value;
    }
  ...
  private final static class RunnableEvents implements Runnable {
    ...
    DbEventDataSourceTTTT owner;

    RunnableEvents(DbEventDataSourceTTTT owner) {
      this.owner = owner;
    }

    @Override
    public void run() {
      Logger.getLogger(Settings.LOGGER).fine("Firing events ''");

 ...
 */
public class DbEventSpringRowSetReader implements RowSetReader, Serializable {
...
//    private SQLCache sqlCache = new SQLCache();
    ...
    //private static final String RESOURCE_URI = "http://localhost:8081/DogodkiWA/resources/event";

    // private Connection connection;
    public DbEventSpringRowSetReader() {
        try {
    ...
    public void readData(RowSetInternal caller) throws SQLException {
        if (client == null) {
            ...
            webResource = client.resource(ConnectionManager.getInstance().getProperty(ConnectionManager.DB_PRIMARY_WS));
        }
        ...
        try {
            ...
            int par = 0;
            String eventID = null;
            Object event = param.get(par++);
            if (event != null) {
                if (event instanceof Long) {
                    eventID = Long.toString((Long) event);
                } else if (event instanceof Integer) {
                    eventID = Integer.toString((Integer) event);
                }
            }
            if (eventID != null) {
                webResource = webResource.queryParam("eventId", eventID);
            }
            Integer idSifranta = (Integer) param.get(par++);

            if (idSifranta != null) {
                webResource = webResource.queryParam("idSifranta", Integer.toString(idSifranta));
            }
            String idSifre = (String) param.get(par++);

            if (idSifre != null) {
                webResource = webResource.queryParam("idSifre", idSifre);
            }

            com.openitech.xml.wrs.WebRowSet xml = webResource.accept(MediaType.APPLICATION_XML).get(com.openitech.xml.wrs.WebRowSet.class);
            if (wrs instanceof DbWebRowSetImpl) {
                ((DbWebRowSetImpl)wrs).readXml(xml);
            } else {
            //to ni cist tko
                   //              wrs.readXml(new StringReader(xml.toString()));

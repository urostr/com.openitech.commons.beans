    ...
    try {
      ...
      if (event != null) {
        if (event instanceof Long) {
          eventID = (int)((Long) event).longValue();
      ...
      wsParameters.setUrl(ConnectionManager.getInstance().getProperty(ConnectionManager.DB_ENTRY_SERVICE));

      int paramIndex = 1;
      WSParameters.Parameter parameter = new WSParameters.Parameter();
      parameter.setIndex(paramIndex++);
      parameter.setValue(eventID);
      ...
      com.openitech.xml.wrs.WebRowSet xml = UtilitiesWS.getInstance().callWebServiceBean(Beans.EVENT_DATASOURCE, Command.ROWSET, wsParameters, com.openitech.xml.wrs.WebRowSet.class);
